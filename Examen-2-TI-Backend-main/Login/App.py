...
def Index():
    ...
    cur.execute('SELECT *FROM user')#hacemos la consutla sql para traer todos los datos
    ...
    return render_template('index.html',user=data)#enviara esta imagen, renderizara el formulario
    #el horarios=data es para poder mandar al html las tuplas creadas
@app.route('/add_contact',methods=['POST'])
def add_contact():# empezaremos a guardar datos
    if request.method=='POST':
        cui=request.form['cui']#guardamos en una variable lo recibido desde el form
        password=request.form['password']#recibimos cada dato guardandolos en una variable
        ...
        cur.execute('INSERT INTO user(cui,password) VALUES(%s,%s)',(cui,password))
...
def get_contact(id):#solo obtendremos un contacto para poder modificarlo
    cur= mysql.connection.cursor()
    cur.execute('SELECT *FROM user WHERE id= %s',(id))#mediante el id haremos la ejecucion de sql para traer esa fila 
    data=cur.fetchall()#me traera el arreglo de la fila del id respectivo
    return render_template('edit_usuario.html',users=data[0])
...
def delete_contact(id):
    ...
    cur=mysql.connection.cursor()
    cur.execute('DELETE FROM user WHERE id={}'.format(id))#el format nos sirve para poder remplazar en la instruccion sql el id
